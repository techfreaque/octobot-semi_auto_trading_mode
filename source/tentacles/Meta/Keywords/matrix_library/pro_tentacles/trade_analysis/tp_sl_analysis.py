    ...
    if maker.strategy_cache:
        for timestamp in maker.strategy_cache:
            for strategy_id in maker.strategy_cache[timestamp]:
                strategy_trading_side = maker.strategy_cache[timestamp][strategy_id]
        ...
        if final_long_whitelist:
            lows = await public_exchange_data.get_candles_(maker, "low")
            highs = await public_exchange_data.get_candles_(maker, "high")
            opens = await public_exchange_data.get_candles_(maker, "open")
        ...
        if final_short_whitelist:
            lows = lows or await public_exchange_data.get_candles_(maker, "low")
            highs = highs or await public_exchange_data.get_candles_(maker, "high")
            opens = opens or await public_exchange_data.get_candles_(maker, "open")
            times = times or await public_exchange_data.get_candles_(maker, "time")
