...
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.indicators.indicator_handling import (
    get_configurable_indicator,
    activate_configurable_indicator,
)


async def get_is_oversold(maker, evaluator):
    ...
    osc_is_os_val = await user_input2(
        maker,
        evaluator,
        f"show signals if {selected_indicator} is currently below:",
        "int",
        40,
    )
    enable_was_os = await user_input2(
        maker, evaluator, "enable was also below in the past", "boolean", False
    )
    osc_was_os_val = osc_is_os_val
    osc_was_within = 1
    if enable_was_os:
        ...
        if int(osc_was_within) != 0:
            osc_was_os_val = await user_input2(
                maker,
                evaluator,
                "show signals if the oscillator was " "below in the past:",
                "int",
                30,
            )
        else:
    ...
    await allow_enable_plot(
        maker, evaluator, f"Plot when {selected_indicator} is below"
    )
    (
        evaluator.values,
    ...
    for i in range(osc_was_within, indicator_d_length):
        oversold_data = evaluator.values[i] < osc_is_os_val
        oversold_data = (
            oversold_data
            and min(evaluator.values[i + 1 - osc_was_within : i + 1]) < osc_was_os_val
        )
        evaluator.signals.append(1 if oversold_data else 0)

    evaluator.second_values = (
        [osc_is_os_val] * indicator_d_length if evaluator.plot else []
    )
    evaluator.second_title = f"Oversold line ({osc_is_os_val})"
    evaluator.second_chart_location = "sub-chart"

    evaluator.title = f"{indicator_title} is oversold"
