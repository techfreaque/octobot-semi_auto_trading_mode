...
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.indicator_keywords.above_below.was_not_above_below import (
    was_not_below,
)
...
async def get_wasnt_below(maker, evaluator):
    ...
    look_back = await user_input2(
        maker, evaluator, "was not below x candles back", "int", 10, min_val=1
    )
    await allow_enable_plot(
        maker,
        evaluator,
        f"Plot when {above_values_title} was not below {below_values_title}",
    )
    (
        data_above_values,
    ...
    evaluator.signals = was_not_below(
    ...
    evaluator.title = (
        f"{above_values_detailed_title} was not below {below_values_detailed_title}"
    )
