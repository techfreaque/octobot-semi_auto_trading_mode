...
from tentacles.Meta.Keywords.matrix_library.pro_tentacles import (
...
class Evaluator_:
    ...
    def init_paths(self, maker):
        self.config_path = (
            ...
            f"{self.evaluator_id + 1} - {self.name}"
        )
        self.cache_path = (
    ...
    async def init_evaluator(self, maker, ctx, evaluator_id, use_compact_mode=False):
        ...
        if use_compact_mode:
            ...
            self.enabled = await user_inputs.user_input(
                ...
                f"evaluator_{self.evaluator_id + 1}",
                "boolean",
                def_val=True,
...
def evaluator_error_message(evaluator, ctx, maker, error=None):
    message = (
        f"Strategy Maker: Evaluator has no data {evaluator.evaluator_id + 1} "
