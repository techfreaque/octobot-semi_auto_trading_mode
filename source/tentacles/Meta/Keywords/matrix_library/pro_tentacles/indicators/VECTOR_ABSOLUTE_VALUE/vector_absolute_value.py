...
async def get_vector_absolute_value(maker, indicator, evaluator):
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot vector absolute value")
    candle_data = await get_candles_(maker, candle_source)
    data = tulipy.abs(candle_data)
    data_source = {
        "v": {
            "title": "vector absolute value",
            "data": data,
