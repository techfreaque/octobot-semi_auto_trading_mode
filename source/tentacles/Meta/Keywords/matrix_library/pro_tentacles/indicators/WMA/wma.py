...
async def get_WMA(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "WMA length", "int", 50, 0)

    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select WMA Candle Source"
    )
    await allow_enable_plot(maker, indicator, "Plot EMA")
    data = tulipy.wma(await get_candles_(maker, candle_source), length)
    data_source = {
        "v": {
            "title": f"WMA {candle_source}-{length}",
            "data": data,
