...
async def get_chande_momentum_oscillator(maker, indicator, evaluator):
    length = await user_input2(
        maker, indicator, "Chande Momentum Oscillator length", "int", 50, 0
    )
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot Chande Momentum Oscillator")
    candle_data = await get_candles_(maker, candle_source)
    data = tulipy.cmo(candle_data, length)
    data_source = {
        "v": {
            "title": f"Chande Momentum Oscillator {length}",
            "data": data,
