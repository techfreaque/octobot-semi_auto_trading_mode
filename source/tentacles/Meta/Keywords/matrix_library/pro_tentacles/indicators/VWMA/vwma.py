...
async def get_VWMA(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "VWMA length", "int", 50, 0)
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select VWMA Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot VWMA")
    candle_data = await get_candles_(maker, candle_source)
    volume_data = await get_candles_(maker, "volume")
    data = tulipy.vwma(candle_data, volume_data, length)[length:]
    data_source = {
        "v": {
            "title": f"VWMA {length}",
            "data": data,
