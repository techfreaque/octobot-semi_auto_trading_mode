...
async def get_ADX(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "ADX length", "int", 50, 0)
    await allow_enable_plot(maker, indicator, "Plot ADX")
    data = tulipy.adx(
    ...
    data_source = {
        "v": {"title": f"ADX {length}", "data": data, "chart_location": "sub-chart"}
    }
