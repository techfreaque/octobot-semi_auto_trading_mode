...
async def get_DEMA(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "DEMA length", "int", 50, 0)
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select DEMA Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot DEMA")
    data = tulipy.dema(await get_candles_(maker, candle_source), length)[length:]
    data_source = {
        "v": {
            "title": f"DEMA {length}",
            "data": data,
