...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.indicators.supported_indicators as supported_indicators
...
# required
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.indicators as indicators_module
...
class Indicator:
    ...
    async def init_indicator(
        ...
        self.config_path_short = f"{evaluator.config_path_short}-i{indicator_id}"
        await self.get_shared_indicator_config(maker, evaluator, data_source_name)

    async def get_shared_indicator_config(
        self, maker, parent_evaluator, data_source_name
    ):
        ...
        if new_evaluator_id == "this indicator":
            await basic_keywords.user_input(
                ...
                "object",
                def_val=None,
                title=f"Indicator {self.indicator_id} - {self.indicator_name}"
                f" - {data_source_name}",
                parent_input_name=parent_evaluator.config_path_short,
                editor_options={"grid_columns": 12, "collapsed": True},
            )
