...
async def get_TEMA(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "TEMA length", "int", 50, 0)
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select TEMA Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot TEMA")
    data = tulipy.tema(await get_candles_(maker, candle_source), length)[length:]
    data_source = {
        "v": {
            "title": f"TEMA {length}",
            "data": data,
