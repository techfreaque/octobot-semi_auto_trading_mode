...
async def get_SMA(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "SMA length", "int", 50, 0)
    candle_source = await user_select_candle_source_name(
        maker, indicator, "Select SMA Candle Source", enable_volume=True
    )
    await allow_enable_plot(maker, indicator, "Plot SMA")
    candle_data = await get_candles_(maker, candle_source)
    data = tulipy.sma(candle_data, length)
    data_source = {
        "v": {
            "title": f"SMA {length}",
            "data": data,
