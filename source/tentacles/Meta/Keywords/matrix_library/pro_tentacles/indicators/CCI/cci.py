...
async def get_CCI(maker, indicator, evaluator):
    length = await user_input2(maker, indicator, "CCI length", "int", 50, 0)
    await allow_enable_plot(maker, indicator, "Plot CCI")
    cci_data = tulipy.cci(
    ...
    data_source = {
        "v": {"title": f"CCI {length}", "data": cci_data, "chart_location": "sub-chart"}
    }
