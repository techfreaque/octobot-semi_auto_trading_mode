...
from octobot_trading.modes.script_keywords import basic_keywords, context_management
import tentacles.Meta.Keywords.matrix_library.basic_tentacles.matrix_basic_keywords.matrix_enums as matrix_enums
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.backtesting import (
    skip_runs as skip_runs,
)
...
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.strategy_maker import (
    strategy_builder_enums as strategy_builder_enums,
)
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.strategy_maker import (
    init_strategy,
)
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.trade_analysis import (
    trade_analysis_activation as trade_analysis_activation,
)
...
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.evaluators import (
    supported_evaluators as supported_evaluators,
)
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.trade_analysis.trade_analysis_activation import (
    handle_trade_analysis_for_backtesting_first_candle,
)
...
class StrategyMakingBaseProducer(
    producer_base.MatrixProducerBase,
):
    def __init__(self, channel, config, trading_mode, exchange_manager):
        super(StrategyMakingBaseProducer, self).__init__(
    ...
    async def build_strategies_backtesting_cache(self, plot_only_mode=False):
        ...
        self.all_timestamps = await public_exchange_data.get_candles_(self, "time")
        for strategy_id in range(0, self.nr_of_strategies):
    ...
    async def handle_plot_signals(self):
        if self.plot_signals:
            ...
            for timestamp, strategies in self.strategy_cache.items():
                for strategy_id, strategy_side in strategies.items():
                    if strategy_id not in signals_by_strategies:
                        signals_by_strategies[strategy_id] = [[], []]
                    signal_value = None
                    if strategy_side == "l":
                        signal_value = (
                            await public_exchange_data.get_candle_from_time(
                                self,
                                timestamp=timestamp,
                                source_name="low",
                            )
                    ...
                    signals_by_strategies[strategy_id][1].append(signal_value)
