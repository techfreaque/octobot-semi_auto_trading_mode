...
import uuid
import tentacles.Meta.Keywords.matrix_library.basic_tentacles.matrix_basic_keywords.matrix_errors as matrix_errors
from tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.daemons.ping_pong import (
    ping_pong_constants,
)
...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.settings.position_size_settings as size_settings
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.matrix_constants as matrix_constants
...
async def get_manged_order_position_size(
    ...
    (max_position_size, max_buying_power, position_size,) = await adapt_sizes_to_limits(
        maker.ctx,
        position_size,
        max_position_size,
        entry_side,
...
def get_managed_order_tags(maker, position_size_settings):
    ...
    tp_order_tag = (
        f"{ping_pong_constants.TAKE_PROFIT}{matrix_constants.TAG_SEPERATOR}"
        f"{position_size_settings.managed_order_group_id}{matrix_constants.TAG_SEPERATOR}{order_tag_id}"
    )
    sl_order_tag = (
        f"{ping_pong_constants.STOP_LOSS}{matrix_constants.TAG_SEPERATOR}"
        f"{position_size_settings.managed_order_group_id}{matrix_constants.TAG_SEPERATOR}{order_tag_id}"
    )
    entry_order_tag = (
        f"{ping_pong_constants.ENTRY}{matrix_constants.TAG_SEPERATOR}"
        f"{position_size_settings.managed_order_group_id}{matrix_constants.TAG_SEPERATOR}{order_tag_id}"
    )
