...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.daemons.ping_pong.ping_pong_constants as ping_pong_constants
...
class PingPongSingleData:
    ...
    def __init__(
        ...
        if not init_only:
            ...
            self.ping_pong_active = bool(entry_order)
    ...
    def log_replaced_entry_order(
        ...
        try:
            ...
            get_logger("PingPongStorage").exception(
    ...
    def get_to_replace_order_details(self):
        ...
        tag_suffix = f"{matrix_constants.TAG_SEPERATOR}{self.entry_counter}"
...
def convert_order_object_to_dict(orders_dict: dict):
    if not orders_dict["entry_orders"]:
        return {
            ping_pong_constants.PingPongSingleDataColumns.ENTRY_ORDER: {},
            ping_pong_constants.PingPongSingleDataColumns.EXIT_ORDERS: {},
        }
...
def order_to_dict(order):
    ...
    order_dict[trading_enums.ExchangeConstantsOrderColumns.FILLED_AMOUNT.value] = float(
        str(order_dict[trading_enums.ExchangeConstantsOrderColumns.FILLED_AMOUNT.value])
    )
    return order_dict
