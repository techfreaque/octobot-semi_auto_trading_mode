...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.stop_losses.trailing_types as trailing_types
...
async def trail_stop_losses(maker, order_group_settings, order_settings):
    if (
        ...
        if _open_orders:
            ...
            for order in _open_orders:
                if order.tag and order.tag.startswith(
                    order_group_settings.order_tag_prefix
                ):
                    new_sl_price = None
                    trading_side = (
...
async def adjust_managed_stop_loss(maker, managed_orders_settings, managed_order_data):
    ...
    if init_stop_order:
        if init_stop_order.origin_price != new_sl_price:
            try:
            ...
            except Exception as error:  # fails if we try to set an SL above the current price
                ...
                try:
                    ...
                    managed_order_data.sl_in_d = float(new_sl_price)
                except Exception as error:  # catch all errors to continue placing take profits
                    maker.ctx.logger.exception(
                        error,
                        True,
