...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.calculators.position_sizing as position_sizing
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.calculators.stop_loss as stop_loss
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.calculators.take_profit as take_profit
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.daemons.ping_pong.ping_pong_storage.storage as storage
...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.settings.entry_types as entry_types
...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.managed_order_pro.utilities as matrix_utilities
...
import tentacles.Meta.Keywords.matrix_library.pro_tentacles.pro_keywords.orders.scaled_orders as scaled_orders
...
class ManagedOrderPlacement:
    def __init__(self):
        ...
        self.group_orders_settings: all_settings.ManagedOrdersSettings = None
        ...
        self.entry_side: str = None
        self.exit_side: str = None
        ...
        self.trading_side: str = None

        self.created_orders: list = None
        self.calculated_entries: list = None
    ...
    async def place_managed_entry_and_exits(
        ...
        # entry grid limits
            ...
            for grid in self.group_orders_settings.entry.entry_grids.values():
                ...
                if self.place_entries and not order_preview_mode:
        ...
        if (
            ...
            return
        if self.group_orders_settings.ping_pong.ping_pong_mode_enabled:
            ...
            ping_pong_storage.set_ping_pong_data(
                ...
                order_group_id=self.order_tag_id,
    ...
    async def plot_order_preview(self, maker):
        if self.calculated_entries:
            ...
            await ctx.reset_cached_values(["ntry_pr", "stp_pr", "tp_pr", "amt_pr"])
    ...
    async def get_single_order_data(
        self, maker, group_orders_settings, entry_price, entry_side
    ):
